...
var haxfred_irc = function(haxfred) {
  ...
  Haxfred.irc.client.addListener('nick', function (oldnick, newnick, channels, msgObj) {
    
    for(var channel in channels) {
      var ch = channels[channel];
      var removeNick = Haxfred.irc.users[ch].indexOf(oldnick);
      if(removeNick > -1) {
        Haxfred.irc.users[ch].splice(removeNick, 1);
      }
      Haxfred.irc.users[ch].push(newnick);
    }

    Haxfred.irc
     haxfred.emit('irc.nick',{
        channels: channels,
        oldnick: oldnick,
        newnick: newnick,
        message: msgObj,
